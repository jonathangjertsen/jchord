...
def test_spreader():
    ...
    assert Spreader(amount=10, jitter=10).apply(notes([1,2,3,4])) == [
        PlayedNote(time=pytest.approx(6.888437030500962), note=1, duration=0, velocity=0),
        PlayedNote(time=pytest.approx(15.159088058806049), note=2, duration=0, velocity=0),
        PlayedNote(time=pytest.approx(18.411431616616902), note=3, duration=0, velocity=0),
        PlayedNote(time=pytest.approx(25.178335005859267), note=4, duration=0, velocity=0),
    ]

def test_chain():
    assert Chain(
        Doubler(12),
        Spreader(amount=10, jitter=0),
        Inverter()
    ).apply(notes([1,2,3,4,12])) == [
